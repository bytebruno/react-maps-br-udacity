...
class Map extends Component {
  ...
  loadMarkers(map) {
    ...
    CORSRequest.onload = () => {
      if(CORSRequest.status === 200) {
        ...
        this.setState({requestCorsWasSuccessful: false});
      }      
    };
    CORSRequest.onerror = () => {
      this.setState({requestWasSuccessful: false});
  ...
  render() {
    const {map, places, requestWasSuccessful, requestCorsWasSuccessful} = this.state;

    return (
      requestWasSuccessful && requestCorsWasSuccessful ? (
        ...
        <div>
          {!requestWasSuccessful ? <h1>Falha no carregamento da API de mapas. Tente novamente</h1> : ''}
          {!requestCorsWasSuccessful ? <h1>Falha no carregamento da API do foursquare. Tente novamente</h1> : '' }
        </div>
      )
    )
  }
